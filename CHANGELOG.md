...
### Fixed
- `manifest.json` was still declaring `"version": "1.3.0"` after the 1.4.0
  release. The `pom.xml` and the Maven artifact were correctly bumped, but
  the Confluent-Hub-style manifest stayed behind, causing the plugin to be
  reported with the wrong version by Confluent Hub UI and any tool that
  inspects the manifest. Bumped to `1.4.1` so all version references align.
